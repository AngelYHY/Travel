...
public class GetAllIsEvaluatedao {
	...
	public List<ProductOrder> getallisevaluate(String accountname){
		...
		try {
		statement = connection.prepareStatement("select * from product_order where account_name_fk='"+accountname+"' and is_pay="+1+" and is_back_product="+1+" and is_receive="+1+" and is_evaluate="+0+"");
		...
		while (resultSet.next()) {
			...
			ProductOrder bean=new ProductOrder(product_count, order_id, product_id, receive_id, account_name, order_time, 1, pay_time, 1, back_time, 1, receive_time, 0, buyer_message, null, express_number);
			list.add(bean);
